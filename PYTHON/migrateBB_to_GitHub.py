...
# IMPORTANT COMMANDS!!!
# auth=('fourndo', 'c0f322eef3181b0663bca20f8d176f2a9a01230b')
# url =
# requests.post(url, json.dumps(issue), auth=auth)
fileDir = "C:\\Users\\DominiqueFournier\\Desktop\\GIFtoolsIssues\\Bitbucket"

with open(fileDir + '\\db-1.0.json') as json_data:
    d = json.load(json_data)

BBissues = d['issues']
BBcomments = d['comments']


gh_auth = ('fourndo', 'c0f322eef3181b0663bca20f8d176f2a9a01230b')
github_repo = 'https://api.github.com/repos/giftools/giftools/issues'

# Verify GH creds work
gh_repo_url = github_repo
gh_repo_status = requests.head(gh_repo_url, auth=gh_auth).status_code
...
def get_issue_comments(issue_id, comments):
...
def convert_issue(issue):
    ...
    if issue['status'] not in ('open', 'new', 'on hold'):
        state = 'closed'
    else:
        state = 'open'
    out = {
...
def convert_date(bb_date):
    ...
    date = re.search(r'(\d\d\d\d-\d\d-\d\d)', bb_date)
    time = re.search(r'(\d\d:\d\d:\d\d)', bb_date)
    if date:
        return '{}T{}Z'.format(date[0], time[0])
...
def push_github_issue(issue, comments, github_repo, auth):
    """
    ...
    # url = github_repo
    # respo = requests.post(url, json=issue_data, auth=auth)
    respo = requests.post(github_repo, json.dumps(issue), auth=auth)
    ...
    if respo.status_code == 201:
        ...
        for comment in comments:
            requests.post(github_repo+'/' + str(iID) + '/comments', json.dumps(comment), auth=gh_auth)

        if issue['state'] == 'closed':
        
            requests.patch(github_repo+'/'+str(iID), json.dumps({'state':'closed'}), auth=gh_auth)
...
for index, issue in enumerate(BBissues[6:]):
    comments = get_issue_comments(issue['id'], BBcomments)

    gh_issue = convert_issue(issue)
    gh_comments = [convert_comment(c) for c in comments
                   if convert_comment(c) is not None]

    gh_issue['title'] = gh_issue['title'] + ' (BB->Imported)'
    # if options.dry_run:
    #     print("\nIssue: ", gh_issue)
    #     print("\nComments: ", gh_comments)
    # else:
    push_github_issue(gh_issue, gh_comments, github_repo, gh_auth)
