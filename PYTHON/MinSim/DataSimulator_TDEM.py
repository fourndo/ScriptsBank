...
out_dir = 'C:\\LC\\Private\\dominiquef\\Projects\\4414_Minsim\\Modeling\\VPmg\\FWR_TDEM\\INP_DIR'
...
core = {'DC':np.r_[np.ones(100)*dx[0]],
        'TDEM':np.r_[np.ones(5)*dx[1],[33,26], np.ones(10)*dx[0],[26,33],np.ones(5)*dx[1]],
        'Gz':np.r_[np.ones(15)*dx[1], [20,15], np.ones(nCx)*dx[0], [15,20], np.ones(15)*dx[1]],
        'MAG':np.r_[np.ones(15)*dx[1], [20,15], np.ones(nCx)*dx[0], [15,20], np.ones(15)*dx[1]]}
        
ndv = {'MAG':0, 'Gz':0, 'DC':1e-8, 'FDEM':1e-8, 'TDEM':1e-8}

npadxy = {'DC':10,'TDEM':7, 'Gz':7, 'MAG':7  }
npadz = {'DC':10, 'TDEM':10, 'Gz':10, 'MAG':10 }

gCx = np.asarray(range(36))*lenTile + offTile + mesh_global.x0[0]
gCy = np.asarray(range(92))*lenTile + offTile + mesh_global.x0[1]
...
#pdis = np.sum(dx[0]*expf**(np.asarray(range(npad))+1))
...
##    E_pad = (x0[ii] + nCx*dx + np.cumsum(padf)) < mesh.vectorNx[-1]
##    
##    S_pad = (y0[ii] - np.cumsum(padb)) > mesh.vectorNy[0]
##    N_pad = (y0[ii] + nCx*dx + np.cumsum(padf)) < mesh.vectorNy[-1]
#    
#    # Create local mesh
#    hx = np.r_[padb, core, padf]
#    hy = np.r_[padb, core, padf]
...
#    
#    mtemp._x0 = (mtemp.x0[0] + X[ii],mtemp.x0[1]+Y[ii], mesh.x0[2]-1000.)
#    meshes.append(mtemp)
