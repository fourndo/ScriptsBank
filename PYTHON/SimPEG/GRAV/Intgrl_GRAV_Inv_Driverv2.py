...
# Load mesh file
mesh = Mesh.TensorMesh.readUBC(home_dir + dsep + mshfile)

# Load in observation file
survey = PF.Gravity.readUBCgravObs(home_dir + dsep + obsfile)
...
# Write result
Mesh.TensorMesh.writeModelUBC(mesh,home_dir + dsep + 'SimPEG_inv_l2l2.sus',m_out)
...
opt = Optimization.ProjectedGNCG(maxIter=10 , maxIterLS = 20,lower=bounds[0],upper=bounds[1], maxIterCG= 50, tolCG = 1e-4)
...
save_log.fileName = home_dir + dsep + 'LogName_blabla'
...
Mesh.TensorMesh.writeModelUBC(mesh,home_dir + dsep + 'SimPEG_inv_l0l2.sus',m_out)
