...
# Create mesh
hxind = [(dx_in,18,-1.3), (dx_in, nx), (dx_in,18,1.3)]
...
dat1 = mesh.plotSlice(model, ind=-15, normal='Z', grid=False, pcolorOpts={'alpha':0.5}, ax =ax_prim)
#==============================================================================
...
ph = DC.plot_pseudoSection(survey2D,axs,stype=stype, dtype = dtype, clim = [vmin,vmax])
plt.gca().set_aspect('equal', adjustable='box')
...
for jj in range(2):
    ...
    #Load model
    invmod.append( DC.readUBC_DC2DModel(inv_dir + dsep + 'dcinv2d.con') )
    ...
    DC.plot_pseudoSection(DCtemp,axs,stype ='pdp', clim = (ph.get_clim()[0],ph.get_clim()[1]), colorbar=False)
...
for jj in range(2):
    ...
    if jj == 1:
        ...
        cbar = fig.colorbar(ph,cax=cbarax, orientation="horizontal", ax = axs, ticks=np.linspace(vmin,vmax, 4), format="$10^{%.1f}$")
