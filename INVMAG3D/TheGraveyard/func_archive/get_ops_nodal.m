function [AVEN,AVCE,AVCN,AVFC,AVEC,GRAD,DIV,DIVbc,CURL,Q,R,RI,D,Sz,Sx,Sy,H0,Wr,V] = get_ops_nodal(X0, Y0, Z0, dx, dy, dz, ObsX, ObsY, ObsZ, H, I, D)
...
avfcx = av(nx); %avenx([1,end]) = 1;
avfcy = av(ny); %aveny([1,end]) = 1;
avfcz = av(nz); %avenz([1,end]) = 1;

Avfcx = kron ( kron ( speye (ny) , avfcx ), speye (nz) );
Avfcy = kron ( kron ( avfcy , speye (nx) ), speye (nz) );
Avfcz = kron ( kron ( speye (ny) , speye (nx)), avfcz );
...
%% Averaging operator edge to center
av = @(n) spdiags (ones (n+1,1)*[0.5,0.5],[0,1],n,n+1);

avefy = av(nx); %avenx([1,end]) = 1;
avefx = av(nz); %aveny([1,end]) = 1;
avefz = av(nx); %avenz([1,end]) = 1;

Avecz = Avfcy * kron ( kron ( speye (ny+1) , avefx ), speye (nz) );
Avecx = Avfcz * kron ( kron ( avefy , speye (nx) ), speye (nz+1) );
Avecy = Avfcx * kron ( kron ( speye (ny) , speye (nx+1)), avefz );
...
%% CURL - Derivative matrices for curl (edges to faces)
ddx = @(n) spdiags (ones (n+1,1)*[1,-1],[0,1],n+1,n+2);

d_dz = dZ * ddx(nz-1); %d_dz=d_dz(:,2:end-1); 
Dzy = kron( kron( speye(ny) , speye(nx+1) ), d_dz );

d_dy = dY * ddx(ny-1); %d_dy=d_dy(:,2:end-1); 
Dyz = kron( kron( d_dy , speye(nx+1)  ), speye(nz));

d_dz = dZ * ddx(nz-1); %d_dz=d_dz(:,2:end-1); 
Dzx = kron( kron( speye(ny+1) , speye(nx)) , d_dz );

d_dx = dX * ddx(nx-1); %d_dx=d_dx(:,2:end-1); 
Dxz = kron( kron( speye(ny+1) , d_dx ), speye(nz) );

d_dy = dY * ddx(ny-1); %d_dy=d_dy(:,2:end-1);
Dyx = kron( kron( d_dy , speye(nx) ), speye(nz+1) );

d_dx = dX * ddx(nx-1); %d_dx=d_dx(:,2:end-1); 
Dxy = kron( kron( speye(ny) , d_dx ), speye(nz+1) );

Ox = sparse ( nfx , nex );
Oy = sparse ( nfy , ney );
Oz = sparse ( nfz , nez );

CURL = [Oz Dyx -Dxy;-Dyz Ox Dzy;Dxz -Dzx Oy];
