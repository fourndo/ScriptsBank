...
addpath ..\FUNC_LIB\;
% Project folders
work_dir = 'C:\Users\dominiquef.MIRAGEOSCIENCE\Google Drive\Research\Modelling\Topo_adjust\Sphere_model';
obsfile = 'Sphere_2pc_0p1floor.dat';
...
%                     [MOF,aVRWs,aVRWx,aVRWy,aVRWz] = get_lp_MOF(invmod,V,Ws,Vx,Wx,Vy,Wy,Vz,Wz,alpha,pvec(pp),qvec(qq),lvec(ll));                   
               ...
               %% Step length, line search                
                ...
                while phi_temp > phi(end) || gamma == 2
                    ...
                    lowb(nullcell==1) = m_temp(nullcell==1) <= 0;
                ...
                fprintf('phid:\t %8.5e\n',phi_d(count))
                fprintf(fid,' \t %8.5e ',invmod'*alpha(1)*WstWs*invmod);
                fprintf(fid,' \t %8.5e ',invmod'*alpha(2)*WxtWx*invmod);
                fprintf(fid,' \t %8.5e ',invmod'*alpha(3)*WytWy*invmod);
                fprintf(fid,' \t %8.5e ',invmod'*alpha(4)*WztWz*invmod);
               ...
               % Output interation result
                ...
                save([work_dir '\RTC_G_' 'p' num2str(pvec(pp)) 'q' num2str(qvec(qq)) 'l' num2str(lvec(ll)) '_iter_' num2str(count) '.sus'],'-ascii','model_out')
                write_MAG3D_TMI(work_dir,['\RTC_G_iter_' num2str(count) '.pre'],H,I,Dazm,obsx,obsy,obsz,(G*invmod).*wd,wd)
