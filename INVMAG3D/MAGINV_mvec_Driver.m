...
% Project folders
work_dir = 'C:\Users\dominiquef.MIRAGEOSCIENCE\Google Drive\Research\Modelling\Synthetic\Triple_Block';
inpfile   = 'MAGvecB.inp'; 
...
Esus = kron( speye(3),spdiags(esus,0,mcell,mcell) );

P = kron( speye(3),spdiags(nullcell.*(esus/max(esus)).^2,0,mcell,mcell) );

% Initialize dynamic cells
Di = kron( speye(3), spdiags(nullcell,0,mcell,mcell));

% Load observation file (3C UBC-MAG format)
[H, I, Dazm, D, obsx, obsy, obsz, dtot, wd] = read_MAG3D_obs([work_dir '\' obsfile]);
        ...
        for qq = 1:length(qvec)   
            ...
            while phi_d(end)>target
                    ...
                    if count==1                 
                        ...
                        [MOF,aVRWs,aVRWx,aVRWy,aVRWz] = get_lp_MOF_mGRAD(invmod,mref,nx,ny,nz,V,Ws,Vx,Wx,Vy,Wy,Vz,Wz,alphas,2,2,1,FLAG);
                        
                        MOF_start = MOF;

                        if isempty(beta)==1

                            beta = full( sum(sum(G.^2,1)) / sum(diag(MOF,0)) * 1e+3 );
                    ...
                    elseif count~=1
                        ...
                        [MOF,aVRWs,aVRWx,aVRWy,aVRWz] = get_lp_MOF_mGRAD(invmod,mref,nx,ny,nz,V,Ws,Vx,Wx,Vy,Wy,Vz,Wz,alphas,pvec(pp),qvec(qq),lvec(ll),FLAG);
                ...
                model_out = Esus * H * invmod;%M + IWr * Esus * invmod;
...
%                 save([work_dir '\lml_' 'p' num2str(pvec(pp)) 'q' num2str(qvec(qq)) 'l' num2str(lvec(ll)) '.mod'],'-ascii','lml')
