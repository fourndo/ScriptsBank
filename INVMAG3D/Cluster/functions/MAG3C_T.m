function [Tx,Ty,Tz] = MAG3C_T(obsx,obsy,obsz,mcell,celln,cellID)
...
Ty(1,mcell+cellID(:)) = atan( dx1 .* dz2 ./ ( dy2 .* arg1 ) ) +...
...
Ty(1,cellID(:)) = log( ( dz2 + arg2 ) ./ (dz1 + arg3 ) ) +...
        -log( ( dz2 + arg1 ) ./ (dz1 + arg4 ) ) +...
         log( ( dz2 + arg6 ) ./ (dz1 + arg7 ) ) +...
        -log( ( dz2 + arg5 ) ./ (dz1 + arg8 ) );


Tx(1,cellID(:)) = atan( dy1 .* dz2 ./ ( dx2 .* arg5 ) ) +...
...
Ty(1,2*mcell+cellID(:)) = log( ( dx1 + sqrt( dx1.^2 + R1 ) ) ./ (dx2 + sqrt( dx2.^2 + R1 ) ) ) +...
...
Tx(1,2*mcell+cellID(:)) = log( ( dy1 + sqrt( dy1.^2 + R1 ) ) ./ (dy2 + sqrt( dy2.^2 + R1 ) ) ) +...
        -log( ( dy1 + sqrt( dy1.^2 + R2 ) ) ./ (dy2 + sqrt( dy2.^2 + R2 ) ) ) +...
         log( ( dy1 + sqrt( dy1.^2 + R4 ) ) ./ (dy2 + sqrt( dy2.^2 + R4 ) ) ) +...
        -log( ( dy1 + sqrt( dy1.^2 + R3 ) ) ./ (dy2 + sqrt( dy2.^2 + R3 ) ) );

Tz(1,2*mcell+cellID(:)) = -( Ty(1,mcell+cellID(:)) + Tx(1,cellID(:)) );
Tz(1,mcell+cellID(:)) = Ty(1,2*mcell+cellID(:));
Tx(1,mcell+cellID(:)) = Ty(1,cellID(:));
Tz(1,cellID(:)) = Tx(1,2*mcell+cellID(:));
