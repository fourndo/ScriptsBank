...
%         else    
            ...
            rs = 1./ ( abs((m - mref)) .^2 + delta_s(jj).^2 ).^( (1 - lp(ii,1)/2) /2 );

%         end

        
        rx = 1./ ( abs(GRADx) .^2 + delta_xyz(jj).^2 ).^( (1 - lp(ii,2)/2) /2 );
        ry = 1./ ( abs(GRADy) .^2 + delta_xyz(jj).^2 ).^( (1 - lp(ii,3)/2) /2 );
        rz = 1./ ( abs(GRADz) .^2 + delta_xyz(jj).^2 ).^( (1 - lp(ii,4)/2) /2 );
...
% %             eta_z = 1 / max(abs( ( subRz'* ( (subRz *  m) ) ) ) );
        ...
        eta_x = delta_xyz(jj).^(1 - lp(ii,2)/2);
        eta_y = delta_xyz(jj).^(1 - lp(ii,3)/2);
        eta_z = delta_xyz(jj).^(1 - lp(ii,4)/2);
