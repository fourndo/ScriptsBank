function [M,pred_TMI,beta_out,phid_out,switcher] = MVI3D_ATP_Tiled_v3(work_dir,out_dir,dsep,idx,xn,yn,zn,H, HI, HD, obsx, obsy, obsz, G, d, wd,mstart,mref,esus,chi_target,alphas,beta,bounds,LP,t,eps_FLAG,eps_tresh,FLAG1,FLAG2,max_iter,ROT)
% Magnetic Vector Invertion
% Written by: D Fournier 
% Last update: 2014/07/23

% Perform a cartesian step first.
...
invmod      =  ones(3*mactv,1) * 1e-4;%mstart ;
% aa = mstart(1:mactv);
...
%                 for pst = 1 : 3
        ...
        if dphim(end)  < 0.1 && lp_count > 1 && phi_d(count-1) < target * (1+0.25) && phi_d(count-1) > target * (1-0.25)%traffic_s(end)*100 <= 1  && traffic_xyz(end)*100 <= 1
...
%                 phi_p = invmod(1+2*mactv:3*mactv)'*(mof((1+2*mactv:3*mactv),(1+2*mactv:3*mactv))'*mof((1+2*mactv:3*mactv),(1+2*mactv:3*mactv))*invmod(1+2*mactv:3*mactv));
                ...
                phi_t = (invmod(1+mactv:2*mactv)-mref(1+mactv:2*mactv))'*(Ws'*spdiags(wr(1+mactv:2*mactv),0,mactv,mactv)*Ws)*(invmod(1+mactv:2*mactv)-mref(1+mactv:2*mactv)) + invmod(1+mactv:2*mactv)'*(Gx'*Wx'*spdiags(wr(1+mactv:2*mactv),0,mactv,mactv)*Wx*Gx + Gy'*Wy'*spdiags(wr(1+mactv:2*mactv),0,mactv,mactv)*Wy*Gy + Gz'*Wz'*spdiags(wr(1+mactv:2*mactv),0,mactv,mactv)*Wz*Gz)*invmod(1+mactv:2*mactv);
                phi_p = (invmod(1+2*mactv:3*mactv)-mref(1+2*mactv:3*mactv))'*(Ws'*spdiags(wr(1+2*mactv:3*mactv),0,mactv,mactv)*Ws)*(invmod(1+2*mactv:3*mactv)-mref(1+2*mactv:3*mactv)) + invmod(1+2*mactv:3*mactv)'*(Gx'*Wx'*spdiags(wr(1+2*mactv:3*mactv),0,mactv,mactv)*Wx*Gx + Gy'*Wy'*spdiags(wr(1+2*mactv:3*mactv),0,mactv,mactv)*Wy*Gy + Gz'*Wz'*spdiags(wr(1+2*mactv:3*mactv),0,mactv,mactv)*Wz*Gz)*invmod(1+2*mactv:3*mactv);
