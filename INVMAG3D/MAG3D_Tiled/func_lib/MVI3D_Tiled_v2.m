function [M,pred_TMI,beta_out,phid_out,MOF_out,switcher] = MVI3D_Tiled(work_dir,out_dir,dsep,idx,xn,yn,zn,H, HI, HD, obsx, obsy, obsz, G, d, wd,mstart,mref,esus,chi_target,phid_in,MOF_in,alphas,beta,bounds,LP,t,eps_FLAG,eps_tresh,switcher,FLAG1,FLAG2,max_iter)
...
% D = mod(450-HD,360);
% TMI = [(cosd(HI) * cosd(D)) (cosd(HI) * sind(D)) sind(HI)];
...
while switcher ~= 3 && count ~= max_iter
    ...
    if switcher == 0     
        ...
        if isempty(beta)==1
            ...
            beta = sum(((Gvec(G,Wd,temp))).^2) / (temp'*MOF*temp) * 1e+4 ;
