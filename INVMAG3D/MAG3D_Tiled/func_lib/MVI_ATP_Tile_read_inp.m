function [meshfile,obsfile,topofile,mstart,mref,m_vec,chi_target,alphas,beta,bounds,lp_MVIvec,lp_tresh,weightfile,FLAG1,FLAG2,tilefile,ROT] = CMI_Tile_read_inp(inputfile)
% Function [work_dir,meshfile,obsfile,wr_flag,chi_target,alphas,beta,pvec,qvec,lvec] = CMI_Tile_read_inp('inputfile')
...
while line~=-1
    ...
    elseif count == 8
        ...
        if isempty(alphas)==1 || length(alphas)~=4
            ...
            fprintf('Requires four numerical values (e.g.--> 1 1 1 1\n')
            break
   ...
   elseif count == 10
        ...
        if isempty(regexp(temp ,'VALUE','match'))==0
            ...
            fprintf('Default bounds on Amplitude: [0 10]\n');
            bounds(1,:) =[0 10];
    ...
    elseif count == 11
        ...
        if isempty(regexp(temp ,'VALUE','match'))==0
            ...
            fprintf('Default bounds on Theta*pi: [-0.5 0.5]\n');
            bounds(2,:) =[-0.5 0.5];
    ...
    elseif count == 12
        ...
        if isempty(regexp(temp ,'VALUE','match'))==0
            ...
            fprintf('Default bounds on Phi*pi: [-1 1]\n');
            bounds(3,:) =[-1 1];
...
%         tilefile = strtrim(arg{1});
    ...
    elseif count == 21
        ...
        fprintf('Objective function rotated strike: %f, dip: %f, plunge: %f\n',ROT(1),ROT(2),ROT(3))
