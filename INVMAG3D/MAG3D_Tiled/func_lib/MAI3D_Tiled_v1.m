function [m_esus,pre_ampB,beta_out,phid_out,MOF_out,switcher] = MAI3D_Tiled(work_dir,out_dir,dsep,idx,xn,yn,zn,H,HI,HD, obsx, obsy,obsz, Tx, Ty, Tz,d,wd,mstart,mref,mag_xyz,w,chi_target,phid_in,MOF_in,alphas,beta,bounds,LP,t,delta_FLAG,delta_tresh,switcher,FLAG1,FLAG2,max_iter,beta_tol,ROT)
...
%         MOF = Wr* MOF*Wr;
        ...
        diagA   = sum(J.^2,1) + beta(count)*spdiags( Wr* MOF*Wr,0)';
        ...
        phi(count) = comp_phi(invmod,Wr*MOF*Wr,beta(count));
        phi_in = phi(end);
...
%             phi_temp(2) = phi_temp(1);
            ...
            phi_out = comp_phi(m_temp,Wr* MOF*Wr,beta(count));
