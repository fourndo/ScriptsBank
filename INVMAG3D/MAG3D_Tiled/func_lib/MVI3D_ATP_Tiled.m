function [M,pred_TMI,beta_out,phid_out,switcher] = MVI3D_ATP_Tiled(work_dir,out_dir,dsep,idx,xn,yn,zn,H, HI, HD, obsx, obsy, obsz, G, d, wd,mstart,mref,esus,chi_target,alphas,beta,bounds,LP,t,eps_FLAG,eps_tresh,FLAG1,FLAG2,max_iter)
...
%                     wr(1:mcell)= (wr(1:mcell)/10.^kk);
            ...
            %         tresh = dkdt(2,delta(count));
                    ...
                    if dphim(end)  < 5 && switcher ~= 0%traffic_s(end)*100 <= 1  && traffic_xyz(end)*100 <= 1
...
%                     MOF = aVRWs'*aVRWs + aVRWx'*aVRWx + aVRWy'*aVRWy + aVRWz'*aVRWz;
            ...
            %     mof = aVRWs'*Wr*aVRWs + aVRWx'*Wr*aVRWx + aVRWy'*Wr*aVRWy + aVRWz'*Wr*aVRWz;
                ...
                while solves <= solves_max && ggdm > 1e-3
            ...
            %         J   =  ;
                   S = sProj(aa,tt,pp);
                    ...
                    wr(1+mcell:2*mcell)= wr(1+mcell:2*mcell)*max(wr(1:mcell))/max(wr(1+mcell:2*mcell))*(pi*1e-6) ;
                    wr(1+2*mcell:3*mcell)= wr(1+2*mcell:3*mcell)*max(wr(1:mcell))/max(wr(1+2*mcell:3*mcell))*(pi*1e-6);
...
%                     else
                        ...
                        scl_t = max(ss(1+mcell:2*mcell));
                        scl_p = max(ss(1+2*mcell:3*mcell));
...
%                     end
                    ...
                    gamma = phi_m(end) /(...
                                ((aVRWs * (invmod-mref ))' * Wr * (aVRWs * (invmod-mref) ))+...
            ...
            %         fprintf('CG iter %i \n',ncg);
                    ...
                    if sum(temp)~=3*mactv
                        ...
                        if dm_i < dm_a
                            dm = dm + rhs_a * dm_i / dm_a / max(abs(g)) ;
                        else
                            dm = dm + rhs_a;
                        end
