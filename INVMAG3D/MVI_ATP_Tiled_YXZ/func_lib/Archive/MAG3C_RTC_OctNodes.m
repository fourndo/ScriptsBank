function [OctNodes] = MAG3C_RTC_OctNodes(Xn,Yn,Zn,cellID,ztopo_n,OctLev)
...
% Pre-allocate space
OctNodes = zeros( mcell , nOctLev , (OctLev(end)+1)^3 * 6 );

% count = 1;
% progress = -1;
% tic
fprintf('START CALCULATING THE NODES FOR TOPOGRAPHIC CELLS\n')
...
for ii = 1 : mcell
    ...
    for jj = 1 : nOctLev
        ...
        edgecell = OctLev(jj)+1;% Number of cells on edge
        ...
        [keep,~,~] = topocheck(OXn,OYn,OZn,topo);
        ...
        % Store nodes
        OctNodes(ii,jj,1:length(temp)) = temp+1e-10;
...
%     if  d_iter > progress
% 
%     fprintf('C %i pct of data in %8.5f sec\n',d_iter,toc)
%     progress = d_iter;
% 
%     end
%     count = count +1;
