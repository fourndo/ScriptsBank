...
% 
%             subplot(1,2,2)
%             [n, xout] =hist((Gx * invmod(1:mactv)),100); hold off
%             bar(xout,n);  
%             set(gca,'yscale','log')
...
%         wr(1:mactv) = wr(1:mactv) / max(wr(1:mactv));
        ...
        atx = sin((((Gx'*(Wx'*Wx)*Gx*pi*tt/2))));
        aty = sin((((Gy'*(Wy'*Wy)*Gy*pi*tt/2))));
        atz = sin((((Gz'*(Wz'*Wz)*Gz*pi*tt/2))));
        
        at = ((atx.^2+aty.^2 + atz.^2)).^0.5;
        
        apx = sin((((Gx'*(Wx'*Wx)*Gx*pi*pp/2))));
        apy = sin((((Gy'*(Wy'*Wy)*Gy*pi*pp/2))));
        apz = sin((((Gz'*(Wz'*Wz)*Gz*pi*pp/2))));
        
        ap = ((apx.^2+apy.^2 + apz.^2)).^0.5;
        
        aVRWx = blkdiag(aVRWx(1:mactv,1:mactv),speye(2*mactv,2*mactv));
        aVRWy = blkdiag(aVRWy(1:mactv,1:mactv),speye(2*mactv,2*mactv));
        aVRWz = blkdiag(aVRWz(1:mactv,1:mactv),speye(2*mactv,2*mactv));
        
        
%         wr(1+mactv:2*mactv) =wr(1:mactv);%  .*at;
%         wr(1+2*mactv:3*mactv) =wr(1:mactv);%  .*ap;
%         if switcher == 0
%         phi_a = (invmod(1:mactv))'*(Ws'*spdiags(wr(1:mactv),0,mactv,mactv)*Ws)*(invmod(1:mactv)) + invmod(1:mactv)'*(Gx'*Wx'*spdiags(wr(1:mactv),0,mactv,mactv)*Wx*Gx + Gy'*Wy'*spdiags(wr(1:mactv),0,mactv,mactv)*Wy*Gy + Gz'*Wz'*spdiags(wr(1:mactv),0,mactv,mactv)*Wz*Gz)*invmod(1:mactv);
%         phi_t = invmod(1+mactv:2*mactv)'*(Gx'*Wx'*spdiags(wr(1+mactv:2*mactv),0,mactv,mactv)*Wx*Gx + Gy'*Wy'*spdiags(wr(1+mactv:2*mactv),0,mactv,mactv)*Wy*Gy + Gz'*Wz'*spdiags(wr(1+mactv:2*mactv),0,mactv,mactv)*Wz*Gz)*invmod(1+mactv:2*mactv);
%         phi_p =    invmod(1+2*mactv:3*mactv)'*(Gx'*Wx'*spdiags(wr(1+2*mactv:3*mactv),0,mactv,mactv)*Wx*Gx + Gy'*Wy'*spdiags(wr(1+2*mactv:3*mactv),0,mactv,mactv)*Wy*Gy + Gz'*Wz'*spdiags(wr(1+2*mactv:3*mactv),0,mactv,mactv)*Wz*Gz)*invmod(1+2*mactv:3*mactv);
        phi_a = (invmod(1:mactv))'*(Ws'*spdiags(wr(1:mactv),0,mactv,mactv)*Ws)*(invmod(1:mactv)) + invmod(1:mactv)'*(Gx'*Wx'*spdiags(wr(1:mactv),0,mactv,mactv)*Wx*Gx + Gy'*Wy'*spdiags(wr(1:mactv),0,mactv,mactv)*Wy*Gy + Gz'*Wz'*spdiags(wr(1:mactv),0,mactv,mactv)*Wz*Gz)*invmod(1:mactv);
        phi_t = (atx'*Wx'*spdiags(wr(1+mactv:2*mactv),0,mactv,mactv)*Wx*atx + aty'*Wy'*spdiags(wr(1+mactv:2*mactv),0,mactv,mactv)*Wy*aty + atz'*Wz'*spdiags(wr(1+mactv:2*mactv),0,mactv,mactv)*Wz*atz);
        phi_p = (apx'*Wx'*spdiags(wr(1+2*mactv:3*mactv),0,mactv,mactv)*Wx*apx + apy'*Wy'*spdiags(wr(1+2*mactv:3*mactv),0,mactv,mactv)*Wy*apy + apz'*Wz'*spdiags(wr(1+2*mactv:3*mactv),0,mactv,mactv)*Wz*apz);
    
        Wr = spdiags(wr,0,3*mactv,3*mactv);

%         g = (aVRWs'*Wr*aVRWs + aVRWx'*Wr*aVRWx + aVRWy'*Wr*aVRWy + aVRWz'*Wr*aVRWz)*invmod;
%         
%         phi_a = (invmod(1:mactv))'*g(1:mactv);
%         phi_t = (invmod(1+mactv:2*mactv))'*g(1+mactv:2*mactv);
...
%         end
        ...
        phim = ( temp'* aVRWs'*Wr*aVRWs * temp ) +...
                ( ([invmod(1:mactv);atx;apx])'*aVRWx'*Wr*aVRWx * ([invmod(1:mactv);atx;apx]) ) +...
                ( ([invmod(1:mactv);aty;apy])'*aVRWy'*Wr*aVRWy * ([invmod(1:mactv);aty;apy]) ) +...
                ( ([invmod(1:mactv);atz;apz])'*aVRWz'*Wr*aVRWz * ([invmod(1:mactv);atz;apz]) );
        ...
        phi_in = phi_d(count) +  beta(count)* gamma * phim;
        ...
        switch FLAG1
            ...
            case 'SMOOTH_MOD'
                ...
                g_MOF = ( aVRWs'*Wr*aVRWs * temp ) +...
                        ( aVRWx'*Wr*aVRWx * ([invmod(1:mactv);atx;apx]) ) +...
                        ( aVRWy'*Wr*aVRWy * ([invmod(1:mactv);aty;apy]) ) +...
                        ( aVRWz'*Wr*aVRWz * ([invmod(1:mactv);atz;apz]) );
...
%%
    ...
    save([work_dir dsep 'Tile' '_Theta.dat'],'-ascii','tt');
    save([work_dir dsep 'Tile' '_Phi.dat'],'-ascii','pp');
