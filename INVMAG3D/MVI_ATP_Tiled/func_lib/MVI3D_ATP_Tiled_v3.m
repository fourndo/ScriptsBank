...
% [ Ws, Gx , Gy, Gz, V, Vx, Vy, Vz ]=get_GRAD_op3D_SQUARE_Kron(dx,dy,dz,nullcell,'FWR');
...
%         wr(1+2*mactv:3*mactv)= 1;%wr(1:mactv);
        ...
        phi_t = invmod(1+mactv:2*mactv)'*(spdiags(wr(1+mactv:2*mactv),0,mactv,mactv)+Gx'*Wx'*spdiags(wr(1+mactv:2*mactv),0,mactv,mactv)*Wx*Gx + Gy'*Wy'*spdiags(wr(1+mactv:2*mactv),0,mactv,mactv)*Wy*Gy + Gz'*Wz'*spdiags(wr(1+mactv:2*mactv),0,mactv,mactv)*Wz*Gz)*invmod(1+mactv:2*mactv);
        phi_p = invmod(1+2*mactv:3*mactv)'*(spdiags(wr(1+2*mactv:3*mactv),0,mactv,mactv)+Gx'*Wx'*spdiags(wr(1+2*mactv:3*mactv),0,mactv,mactv)*Wx*Gx + Gy'*Wy'*spdiags(wr(1+2*mactv:3*mactv),0,mactv,mactv)*Wy*Gy + Gz'*Wz'*spdiags(wr(1+2*mactv:3*mactv),0,mactv,mactv)*Wz*Gz)*invmod(1+2*mactv:3*mactv);
...
%         scl_p = max(invmod(1:mactv)); 
        ...
        if isempty(beta)==1
            ...
            beta = sum(((Gvec(G,Wd,temp))).^2) / (temp'*mof*temp) * 1e+1 ;
...
%         tresh = dkdt(2,delta(count));
        ...
        if lp_count == 1 
            ...
            delta_q(2) = 2.5e-2; % Fix value since always angles
            delta_q(3) = 5e-2;
...
%                 temp(1+2*mactv:3*mactv) = sin(temp(1+2*mactv:3*mactv)); 
    ...
    while solves <= solves_max %&& ggdm > 1e-2
        ...
        phi_m(count) = invmod' *( aVRWs'*Wr*aVRWs + aVRWx'*Wr*aVRWx + aVRWy'*Wr*aVRWy + aVRWz'*Wr*aVRWz) *invmod;
        phi_in = phi_d(count) + beta(count) * phi_m(count);
...
%         wr(1+2*mactv:3*mactv)= wr(1:mactv);%wr(1+2*mactv:3*mactv)/max(wr(1+2*mactv:3*mactv));
        ...
        phi_a = invmod(1:mactv)'*(spdiags(wr(1:mactv),0,mactv,mactv))*invmod(1:mactv);
        phi_t = invmod(1+mactv:2*mactv)'*(spdiags(wr(1+mactv:2*mactv),0,mactv,mactv))*invmod(1+mactv:2*mactv);
        phi_p = invmod(1+2*mactv:3*mactv)'*(spdiags(wr(1+2*mactv:3*mactv),0,mactv,mactv))*invmod(1+2*mactv:3*mactv);
...
%         phi_p = (invmod(1+2*mactv:3*mactv))'*g(1+2*mactv:3*mactv);
        ...
        scl_t = phi_a/((phi_t+phi_p)/2);
%         scl_p = (phi_a/phi_p);
%         scl_t = max(invmod(1:mactv));
%         scl_p = max(invmod(1:mactv));
        wr(1:mactv)= wr(1:mactv);
        wr(1+mactv:end)= wr(1+mactv:end)*scl_t;%/(max(wr(1:mcell))/max(wr(1+mcell:2*mcell))) ;
%         wr(1+2*mactv:3*mactv)= wr(1+2*mactv:3*mactv)*scl_p;%/(max(wr(1:mcell))/max(wr(1+2*mcell:3*mcell)));
...
%     % Create sub-space matrix and grab the right model
        ...
        count_LS = 0;
        while (phi_out >= phi_in) && count_LS < 2
            count_LS = count_LS+1;
...
%             gdm(1+2*mactv:end) = rem(gdm(1+2*mactv:end),pi);
%             gdm(1+mactv:2*mactv) = rem(gdm(1+mactv:2*mactv),pi/2);
