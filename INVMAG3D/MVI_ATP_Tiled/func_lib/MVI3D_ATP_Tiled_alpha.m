...
%         % Re-weighted pseudo sensitivity weighting for each component
        ...
        wr = wr / max(wr);
%         wr(1+mactv:2*mactv)= 1;%wr(1:mactv);
%         wr(1+2*mactv:3*mactv)= 1;%wr(1:mactv);
...
%         tresh = dkdt(2,delta(count));
        ...
        if lp_count == 1 
            ...
            delta_q(2:3) = 1e-1; % Fix value since always angles
...
%         wr(1+mactv:2*mactv)= wr(1:mactv);%wr(1+mactv:2*mactv)/max(wr(1+mactv:2*mactv));
%         wr(1+2*mactv:3*mactv)= wr(1:mactv);%wr(1+2*mactv:3*mactv)/max(wr(1+2*mactv:3*mactv));

        
        
        
        phi_a = (invmod(1:mactv))'*(Ws'*spdiags(wr(1:mactv),0,mactv,mactv)*Ws)*(invmod(1:mactv)) + invmod(1:mactv)'*(Gx'*Wx'*spdiags(wr(1:mactv),0,mactv,mactv)*Wx*Gx + Gy'*Wy'*spdiags(wr(1:mactv),0,mactv,mactv)*Wy*Gy + Gz'*Wz'*spdiags(wr(1:mactv),0,mactv,mactv)*Wz*Gz)*invmod(1:mactv);
        phi_t = invmod(1+mactv:2*mactv)'*(Gx'*Wx'*spdiags(wr(1+mactv:2*mactv),0,mactv,mactv)*Wx*Gx + Gy'*Wy'*spdiags(wr(1+mactv:2*mactv),0,mactv,mactv)*Wy*Gy + Gz'*Wz'*spdiags(wr(1+mactv:2*mactv),0,mactv,mactv)*Wz*Gz)*invmod(1+mactv:2*mactv)+invmod(1+2*mactv:3*mactv)'*(Gx'*Wx'*spdiags(wr(1+2*mactv:3*mactv),0,mactv,mactv)*Wx*Gx + Gy'*Wy'*spdiags(wr(1+2*mactv:3*mactv),0,mactv,mactv)*Wy*Gy + Gz'*Wz'*spdiags(wr(1+2*mactv:3*mactv),0,mactv,mactv)*Wz*Gz)*invmod(1+2*mactv:3*mactv);
...
%         scl_p = max(invmod(1:mactv));
        wr(1:mactv)= wr(1:mactv);
        wr(1+mactv:2*mactv)= wr(1+mactv:2*mactv)*scl_t;%/(max(wr(1:mcell))/max(wr(1+mcell:2*mcell))) ;
        wr(1+2*mactv:3*mactv)= wr(1+2*mactv:3*mactv)*scl_t;%/(max(wr(1:mcell))/max(wr(1+2*mcell:3*mcell)));

        Wr = spdiags(wr,0,3*mactv,3*mactv);

%         atx = spdiags(abs(asin(sin(Gx'*(wr(1+mactv:2*mactv).*(Gx*tt)/2)))),0,mactv,mactv);
%         aty = spdiags(abs(asin(sin(Gy'*(wr(1+mactv:2*mactv).*(Gy*tt)/2)))),0,mactv,mactv);
%         atz = spdiags(abs(asin(sin(Gz'*(wr(1+mactv:2*mactv).*(Gz*tt)/2)))),0,mactv,mactv);
%         atx = abs(asin(sin(Gx'*(wr(1+mactv:2*mactv).*(Gx*tt)/2))));
%         atx = spdiags(atx,0,mactv,mactv);
%         aty = abs(asin(sin(Gy'*(wr(1+mactv:2*mactv).*(Gy*tt)/2))));
%         aty = spdiags(aty,0,mactv,mactv);
%         atz = abs(asin(sin(Gz'*(wr(1+mactv:2*mactv).*(Gz*tt)/2))));
%         atz = spdiags(atz,0,mactv,mactv);
%         at = ((atx.^2+aty.^2 + atz.^2)).^0.5;
       
%         apx = abs(asin(sin(Gx'*(wr(1+2*mactv:end).*(Gx*pp)/2))));
%         apx = spdiags(apx,0,mactv,mactv);
%         apy = abs(asin(sin(Gy'*(wr(1+2*mactv:end).*(Gy*pp)/2))));
%         apy = spdiags(apy,0,mactv,mactv);
%         apz = abs(asin(sin(Gz'*(wr(1+2*mactv:end).*(Gz*pp)/2))));
%         apz = spdiags(apz,0,mactv,mactv);
        
%         apx = spdiags(abs(asin(sin(Gx'*(wr(1+2*mactv:end).*(Gx*pp)/2)))),0,mactv,mactv);
%         apy = spdiags(abs(asin(sin(Gy'*(wr(1+2*mactv:end).*(Gy*pp)/2)))),0,mactv,mactv);
%         apz = spdiags(abs(asin(sin(Gz'*(wr(1+2*mactv:end).*(Gz*pp)/2)))),0,mactv,mactv);
%         apx = speye(mactv);
%         apy = speye(mactv);
%         apz = speye(mactv);
        
%         ap = ((apx.^2+apy.^2 + apz.^2)).^0.5;
        
        scx = blkdiag(speye(mactv),spdiags([1./abs(tt);1./abs(pp)],0,2*mactv,2*mactv));
        scy = blkdiag(speye(mactv),spdiags([1./abs(tt);1./abs(pp)],0,2*mactv,2*mactv));
        scz = blkdiag(speye(mactv),spdiags([1./abs(tt);1./abs(pp)],0,2*mactv,2*mactv));
...
%             tt_low = m_temp(lowb==1 & t_ind);
%             tt_low = 2*pi+mod(tt_low,-2*pi);
            
            pp_high = m_temp(uppb==1 & p_ind);
            pp_high = mod(pp_high,2*pi);
            pp_high(pp_high>pi) = -2*pi+pp_high(pp_high>pi);
%             tt_high = m_temp(uppb==1 & t_ind);
%             tt_high = -2*pi+mod(tt_high,2*pi);
...
%             m_temp(uppb==1 & t_ind) = tt_high;
            ...
            lowb(1+2*mactv:end) = 0;
            uppb(1+2*mactv:end) = 0;
