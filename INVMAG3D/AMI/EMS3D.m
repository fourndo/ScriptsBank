function [esus_3D,M] = EMS3D(work_dir,meshfile,obsfile,topofile,mstart,mref,chi_target,alphas,beta,nlayer,FLAG1)
...
esus_3D = repmat(reshape(esus,nx,ny),1,1,nz);
esus_3D = permute(esus_3D,[3 1 2]); esus_3D = esus_3D(:);
esus_3D(nullcell==0) = -100;

save([work_dir '\EMS_3D.sus'],'-ascii','esus_3D');
...
write_MAG3D_TMI([work_dir '\EMS_tmi.pre'],H,I,Dazm,obsx,obsy,obsz,Ptmi * [bx;by;bz],wd);

write_MAG3D_TMI([work_dir '\EMS_amp.pre'],H,I,Dazm,obsx,obsy,obsz,d_amp,wd);

write_MAG3D_3C([work_dir '\EMS_3C.pre'],H,I,Dazm,obsx,obsy,obsz,bx,by,bz,ones(ndata,1),ones(ndata,1),ones(ndata,1));
...
save([work_dir '\EMS_mvec.fld'],'-ascii','M');
