...
%                         scale_x = zeros(mcell,1);
                    ...
                    aVRWs =  spdiags( sqrt( as *  lvec ),0,mcell,mcell)  * Wr * tRs * Ws ;
                    aVRWx =  spdiags( sqrt( ax * (2.0 - lvec)  ),0,mcell,mcell) * Wr * tRx * Wx ;
...
%             axis square
        ...
        text(.3,0.40,['$\mathbf{p: ' num2str(pQ(rr)) ',\; q:' num2str(qQ(rr)) '}$'],'interpreter', 'latex','FontSize',12)
