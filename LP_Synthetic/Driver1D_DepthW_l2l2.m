...
tol = 0.01;
% Lp-norm parameters
pQ = 2;%0:0.1:2;%[0 1 2];%0%
qQ = 2;%0:0.1:2;%[0 1 2];%0%
...
%             axis square
        ...
        text(.15,0.40,['$\mathbf{p: ' num2str(pQ(ll)) ',\; q:' num2str(qQ(ll)) '}$'],'interpreter', 'latex','FontSize',12)

        text(.15,0.36,['$\mathbf{\phi_d} = ' num2str(round(phi_d(count)*100)/100) '$'],'interpreter', 'latex','FontSize',10)
%         text(.35,0.32,['$\epsilon = 10^{' num2str(round(log10(delta_p(end))*10)/10) '}$'],'interpreter', 'latex','FontSize',10)
        text(.15,0.32,['$\|\mathbf{m - m^*}\|$ = ' num2str(round(model_error*100)/100)],'interpreter', 'latex','FontSize',10)
