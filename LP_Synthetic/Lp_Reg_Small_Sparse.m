...
lp_func = @(W,x,pp,eps) sum(W*(x'.^2./(x'.^2 + eps.^2).^(1-pp/2)),2);
...
for jj = 1 : 2
    ...
    beta_in = norm(G'*G*[x0;y0])/norm([1 0;0 1]*[x0;y0]);
...
for pp = 1 : 2
mcell = length(X(:));
reg = lp_func(speye(mcell),[X(:)';Y(:)'], p(pp) , epsl*100);
...
%         end
        ...
        r0 = wt_func([x_iter,y_iter],p(pp),epsl);
        r2 = wt_func([x_iter,y_iter],2,epsl);
        ...
        scale_l0 = 1/max(r0);
        scale_l2 = 1/max(r2);
%         scale_l0 = 1/norm((R0)*[x_iter;y_iter]);%sqrt(2/(r0'*r0));
%         scale_l2 = 1/norm((R2)*[x_iter;y_iter]);%sqrt(2/(r2'*r2));

        R0 = scale_l0*R0;
        R2 = scale_l2*R2;
        
        phi0 = l2_func(R0,[x_iter y_iter]);
        phi2 = l2_func(R2,[x_iter y_iter]);
             
        gamma0 = 1;
        gamma2 = 1;
%         gamma0 = l2_func([1 0;0 1],[x_iter y_iter])/phi0;
%         gamma2 = l2_func([1 0;0 1],[x_iter y_iter])/phi2;
        
        MOF = R0 + R2;
        
        norm((R0+R2)*[x_iter;y_iter]);

%         R0 = gamma0*R0;
%         R2 = 1*gamma2*R2;
        
%         gamma = norm([1 0;0 1]*[x_iter;y_iter]) / norm(MOF*[x_iter;y_iter]);
        gamma=1;
        R0 = gamma*R0;
        R2 = gamma*R2;
        
        IRLS = l2_func(R0,[X(:) Y(:)]) + l2_func(R2,[X(:) Y(:)]);
        IRLS = reshape(IRLS,size(X,1),size(X,2));

%         reg_objfunc = misfit + beta_in*IRLS;

        phi_in = l2_func(R,[x_iter,y_iter])+ l2_func([1 0;0 1],[x_iter,y_iter]);

%         [yval,xval] = find(reg_objfunc==min(min(reg_objfunc))); 
        m_out = (G'*G + beta_in*(R0 + R2))\(G'*d);
        x_iter = m_out(1);
        y_iter = m_out(2);
...
%         mis_func([x_iter y_iter])
        ...
        phi_out = scale * phi_in;
        plot(x_iter,y_iter,'kx','MarkerSize',8);
    end
...
phil0 = lp_func(speye(mcell),[x1;x2],0,epsl);
phil2 = lp_func(speye(mcell),[x1;x2],2,epsl);
...
% Scaled version
r0 = (wt_func([x1;x2],0,epsl));
r2 = (wt_func([x1;x2],2,epsl));

eta_0 = spdiags(2./sum(r0,2),0,mcell,mcell);
eta_2 = spdiags(2./sum(r2,2),0,mcell,mcell);

phi0 = lp_func(eta_0,[x1;x2],0,epsl);
phi2 = lp_func(eta_2,[x1;x2],2,epsl);
