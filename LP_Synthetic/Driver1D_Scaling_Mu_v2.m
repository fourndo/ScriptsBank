...
%             lvec(z{2}==1) = 2;
        ...
        while switcher ~= 3
                    ...
                    if switcher == 0   %First iteration
                        ...
                        delta_p(count) = max(invmod);%prctile(abs(invmod(invmod ~= 0)),pct_cutoff)*2;
                        gradm = abs(Wx * invmod);
                        delta_q(count) = max(gradm);%abs(prctile(gradm,pct_cutoff)*2);
...
%                             delta_p(count) = eps_p;
        ...
        %                     gradm = Wx * invmod;
        %                     eps_x = std(gradm);
                            q_tresh = eps_q;%prctile(abs(gradm(gradm > 0)),pct_cutoff);
...
%                         if switcher == 1 && delta_p(end)> eps_p %&& switcher == 1 && lp_count>1;%traffic_p(end)*100 > 1 && switcher == 1
%                             
...
%                         if switcher == 1 && delta_q(end)> eps_q %&& switcher == 1 && lp_count>1;%traffic_q(end)*100 > 1 
%                             
...
%                 end               
                ...
                if switcher > 0  && dphi_m(end) < 1 && phi_d(count) > target *0.95 && phi_d(count) < target *1.05 
