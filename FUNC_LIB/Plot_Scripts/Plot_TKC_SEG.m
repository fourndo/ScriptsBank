...
%% Input Files
work_dir = 'C:\Users\dominiquef.MIRAGEOSCIENCE\ownCloud\Research\TKC\DIGHEM_TMI\25m\Mixnorm';

meshfile = '..\Mesh_25m.msh';

model2 = 'Tile1_MAG3D_lplq.sus';
model1 = 'Tile1_MAG3D_l2l2.sus';
...
for ii = 1: length(lvl);
% Get top of model
    m2D = get_model_top(m1,nx,ny,nz,-100,lvl(ii));

    temp = reshape(m2D(padW+1:end-padE,padS+1:end-padN),nx-padE-padW,ny-padN-padS); 
    temp = (temp');

    ax = axes('Position',[0.2 .4 .75 .75]);
    h = imagesc(xx,yy,temp);hold on
    contour(xx,yy,temp,1e-3:5e-3:0.1,'k')
    caxis([0 maxcol]);
...
%     xlabel('$x$', 'interpreter', 'latex','FontSize',14)
    ...
    %% Get top of model
    m2D = get_model_top(m2,nx,ny,nz,-100,lvl(ii));
