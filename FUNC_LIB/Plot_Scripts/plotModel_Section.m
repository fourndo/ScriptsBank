...
% inp_dir = 'C:\Users\dominiquef.MIRAGEOSCIENCE\Google Drive\Tli_Kwi_Cho\Modelling\Inversion\EM\DIGHEM\3D\Real_topo\Inv9_FixBeta_1DRef';
...
mod2_file = 'Ref_1em4\Inv_MOD_iter12.con';
mod1_file = '\Ref_5em4\Inv_MOD_iter7.con';
% mod1_file='DIGHEM_Inv10_Oct2Ten.con';
% mod2_file='DIGHEM_Inv10_Oct2Ten_5em5.con';
meshfile = 'UBC_mesh_small_v3.msh';
...
secfile = 'C:\Users\dominiquef.MIRAGEOSCIENCE\Google Drive\Tli_Kwi_Cho\Modelling\DO27_Section_2008.dat';
...
if ~isempty(mod2_file)
    ...
    DOI = abs(log10(m) - log10(m2)) / abs(log10(1e-4) - log10(5e-4));
    DOI(nullcell==0) = 0;
    DOI = (DOI/max(DOI)).^0.5;
%     DOI(DOI>0.5) = (DOI(DOI>0.5)).^.5;
%     DOI(DOI>0.5) = sqrt(DOI(DOI>0.5));
else
...
view([0 90])
set(gca,'YDir','normal')
ylabel('Northing (m)','FontSize',12,'FontName','Microsoft Sans Serif')
xlabel('Easting (m)','FontSize',12,'FontName','Microsoft Sans Serif')

cc = colormap(cpal(1:crange,:));
caxis([min(cvec) max(cvec)])
% colorbar('SouthOutside')
title('Depth $\approx$ 25 m','interpreter', 'latex','FontName','Microsoft Sans Serif')
...
text(ax_xlim(1),yc(DO27section1),zc(zsection),'B','VerticalAlignment','middle','HorizontalAlignment','right','FontSize',12)
text(ax_xlim(2),yc(DO27section1),zc(zsection),['B' char(39)],'VerticalAlignment','middle','HorizontalAlignment','left','FontSize',12)
...
text(ax_xlim(1),yc(DO18section),zc(zsection),'A','VerticalAlignment','middle','HorizontalAlignment','right','FontSize',12)
text(ax_xlim(2),yc(DO18section),zc(zsection),['A' char(39)],'VerticalAlignment','middle','HorizontalAlignment','left','FontSize',12)

axis equal
axis([ax_xlim(1) ax_xlim(2) min(YY(:)) max(YY(:))])

% if mmax < max(m2D(:))
%     mmax = max(m2D(:));
% end
%% Plot vertical section DO27
ax2 = axes('Position',[0.50 .175 .45 .45]);
...
set(gca,'YDir','normal')
zlabel('Depth (m)', 'interpreter', 'latex','FontSize',12,'FontName','Microsoft Sans Serif')
xlabel('Easting (m)', 'interpreter', 'latex','FontSize',12,'FontName','Microsoft Sans Serif')
...
text(ax_xlim(1),YY(1)-10,max(ZZ(:)),'B','VerticalAlignment','bottom','HorizontalAlignment','right','FontSize',12)
text(ax_xlim(2),YY(1)-10,max(ZZ(:)),['B' char(39)],'VerticalAlignment','bottom','HorizontalAlignment','left','FontSize',12)
...
ax3 = axes('Position',[0.50 .55 .45 .45]);
...
caxis([min(cvec) max(cvec)])
% colorbar('SouthOutside')
text(ax_xlim(1),YY(1)-1,max(ZZ(:)),'A','VerticalAlignment','bottom','HorizontalAlignment','right','FontSize',12)
text(ax_xlim(2),YY(1)-1,max(ZZ(:)),['A' char(39)],'VerticalAlignment','bottom','HorizontalAlignment','left','FontSize',12)
...
set(gca,'Visible','off');
text(0.50,1,barlabel, 'interpreter', 'latex','FontSize',12,'HorizontalAlignment','center','FontName','Microsoft Sans Serif')