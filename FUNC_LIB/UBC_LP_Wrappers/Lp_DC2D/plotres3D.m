...
addpath C:\Users\dominiquef\Dropbox\DOM_Projects\Lp_MAG3D\sliceomatic
work_dir = 'C:\Projects\3796_AGIC_Research\MAG3D\Current\Intrusive\Intrusive_v3';
cd (work_dir); cd ..
...
for oo=range
    ...
    if strcmp(folder_name(1:3),'Inv')==1
    
        cd (folder_list(oo,:));
        
    file_list=ls;
    getp =regexp(folder_name,'lp+(.)+_lq','tokens');
    ip =  find(p==str2num(char(getp{1})));

    getq =regexp(folder_name,'_lq+(.)+_','tokens');
