...
home_dir = 'C:\Projects\3796_AGIC_Research\DCIP2D'   ;
...
for ll = 1% : length(l)
    ...
    for pp = 21% : length(p)
        ...
        for qq = 1%: length(q)
            ...
            dos (['copy dcinv2d.con' ' ' work_dir]);
            dos (['copy dcinv2d.log' ' ' work_dir]);   
            dos (['copy dcinv2d.out' ' ' work_dir]);
            ...
            [ndata,phid,meshfile,obsfile,data,logfile,topofile,topo_check]=read_input;
