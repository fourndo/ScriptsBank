...
home_dir = 'C:\Projects\3796_AGIC_Research\DCIP3D\Intrusive'   ;
UBC_dir = 'C:\Projects\3796_AGIC_Research\DCIP3D\Intrusive\UBC_Results' ;
...
dos (['mkdir ' home_dir '\Workspace2']);

work_dir = [home_dir '\Workspace2'];

cd (home_dir);

if iter_start>=10
    strnum = num2str(iter_start);   
else
...
for ll = 1 : length(l)
    ...
    for pp = 1 : length(p)
        ...
        for qq = 1 : length(q)
            ...
            dos (['del ' home_dir '\Workspace2 /Q']);
            dos (['copy ' UBC_dir '\dcinv3d_' strnum '.con' ' ' work_dir]);
%             dos (['rename ' work_dir '\dcinv3d_' strnum '.con dcinv3d_01.con ']);
...
%% Iteration loop computing weightings
            while oo <= 2 && phid(oo) >= ndata * chifact;
            ...
            % Compute dimension matrix for the GRAD terms (FOR DEV ONLY)
                ...
                phi_cm = Ws'.*logmodel' * (wcm .* (Ws .* Wr.* logmodel));
...
%% Create input file
                ...
                fprintf(fid,'VALUE %10.8f !!! reference model (NULL = calculate)\n',ref_model);
                fprintf(fid,'null !!! active cells file (NULL = all cells are active)\n');
                fprintf(fid,'BOUNDS_NONE !!! BOUNDS_NONE | BOUNDS_CONST bl bu | BOUNDS_FILE file  ! bounds\n');
