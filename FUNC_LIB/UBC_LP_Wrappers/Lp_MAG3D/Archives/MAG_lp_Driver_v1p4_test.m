...
addpath (root_lib);
addpath 'C:\Users\dominiquef\Dropbox\DOM_Projects\Lp_MAG3D\functions\DEV'
%% USER INPUTS
...
%                 wc = 1 ./ ( (abs(Wx*model) ) .^ ( 2 - p(pp) ) + delta );
% 
%                 wcx = spdiags(wc,0,(nX-1)*nY*nZ,(nX-1)*nY*nZ);
%                 
%                 wc = 1 ./ ( (abs(Wy*model) ) .^ ( 2 - p(pp) ) + delta );
% 
%                 wcy = spdiags(wc,0,nX*(nY-1)*nZ,nX*(nY-1)*nZ);
%                 
%                 wc = 1 ./ ( (abs(Wz*model) ) .^ ( 2 - p(pp) ) + delta );
% 
%                 wcz = spdiags(wc,0,nX*nY*(nZ-1),nX*nY*(nZ-1));
%                 
%                 phi_cxyz = model'*(( alphax * Wx' * wcx * Wx +...
%                     alphay * Wy' * wcy * Wy +...
%                     alphaz * Wz' * wcz * Wz)*model);
                ...
                phi_cm = norm(alphac * Ws' * (wcm .* (Ws .* Wr.* model) ));
