...
% Iteration number for the starting model from UBC unconstrained
iter_start = '1';
...
for ll = 1 : length(l)
    ...
    for pp = 1 : length(p)
        ...
        for qq = 1 : length(q)
            ...
            [ndata,beta_in,L_scale,phid]=read_log('maginv3d.log',iter_start);
...
%% Iteration loop computing weightings
            ...
            % Compute dimension matrix for the GRAD terms (FOR DEV ONLY)
                ...
                phi_cm = alphas * (Ws'.*model') * (wcm .* (Ws .* Wr.* model));
...
%% Call maginv3d_newbounds and invert...
                ...
                switch argin 
                    ...
                    [arg,arg,arg,phid(oo),iter]=read_log('maginv3d.log','end');
