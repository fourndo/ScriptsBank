...
def read_UBC_log(Work_dir,Home_dir,iter_start,mcell):
    ...
    fid = open(Work_dir + 'ipinv3d.log','r')
    ...
    while line:
        ...
        if  "lag. multiplier:" in line[:20]:
            beta = float(line[20:])
            
            
        if  "--- Begin logarithmic barrier iterations ---" in line:
        ...
        if  "Reference Model:" in line:
            ...
            if not look_value:
                ...
                ref_model = ones((mcell,1), dtype=float) * float(line[25:])
