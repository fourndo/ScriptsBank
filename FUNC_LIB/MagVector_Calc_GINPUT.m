...
% declination and inclination of anomaly at location specified by the user 
%
% Press enter until you reach the slice of the model that you like.
% Click once with the mouse to select the layer
% Click again to get the magnetization orientation at a given location
%
...
mvec = load([work_dir dsep vecfile]);

mamp = sqrt(sum(mvec.^2,2));
...
% Cycle down the amplitude model and pick a location
temp = [];

ind_2D = null_2_idx(nullcell);
h = set(figure, 'Position', [50 25 1500 1000]);
count = -1;
while isempty(temp)
count = count + 1;
idx = sub2ind(size(nullcell),ind_2D(:,1)+count,ind_2D(:,2),ind_2D(:,3));
...
% [~,idy] = min( abs(yc - Yloc));
% Find the first cell below topo
idz = ind_2D(sub2ind([nx ny],Xloc,Yloc),1)+count;
% 
...
%     
% end
[idz,idx,idy] = ndgrid(idz-1:idz+1,Xloc-1:Xloc+1,Yloc-1:Yloc+1);
...
%     
plot(Xloc,Yloc,'ro');
text(Xloc,Yloc,['M: ' num2str(round(m_exp*10)/10) 'e' num2str(m_pow)],'VerticalAlignment','top','HorizontalAlignment','left')
text(Xloc,Yloc-1,['I: ' num2str(round(avg_I*10)/10)],'VerticalAlignment','top','HorizontalAlignment','left')
text(Xloc,Yloc-2,['D: ' num2str(round(avg_D*10)/10)],'VerticalAlignment','top','HorizontalAlignment','left')

[Xloc,Yloc] = ginput(1);
end    
%     % Write to file
%     fprintf(fid,'%6.0f &\t%7.0f &\t%6.1e $\\pm$ %6.1e &\t%6.1f $\\pm$ %6.1f &\t%6.1f $\\pm$ %6.1f \\\\ \n',...
%     Xloc,Yloc,...
