%%DEMONSTRATEMAGNETICCODE  This file demonstrates the code for spherical
...
%not been compiled, then it is can be very slow. Consequently, this file
%runs the CompileCLibraries function before executing. Note that the
%CompileCLibraries function only needs to be run the first
...
display('Compiling the mex functions.')
CompileCLibraries
...
display('Loading the coefficients for the WMM2010 model.')
[C,S,a,c]=getWMM2010Coeffs();
display('Computing the magnetic flux vector on a grid of points.')
[V,gradV]=spherHarmonicEval(C,S,points,a,c);
B=-gradV;
