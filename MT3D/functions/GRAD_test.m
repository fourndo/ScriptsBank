...
X0 = -1; Y0 = -1; Z0 = -1; %Origin
Xmax = 1; Ymax = 1; Zmax = 1; %Origin
...
d_dx = dXn * ddx(nx); d_dx=d_dx(:,2:end-1); d_dx([1,end])=0;
Dx = kron(kron(speye(nz),speye(ny)),d_dx);

d_dy = dYn * ddx(ny); d_dy=d_dy(:,2:end-1); d_dy([1,end])=0;
Dy = kron(kron(speye(nz),d_dy),speye(nx));

d_dz = dZn * ddx(nz); d_dz=d_dz(:,2:end-1); d_dz([1,end])=0;
...
% DIV test
    ...
    phi = @(x,y,z) ( tanh(x) + tanh(y) + tanh(z) ) .* exp(-10*(x.^2 + y.^2 + z.^2));
    ...
    % ANALYTICAL: V x A
    Bie = (1 - tan(Xif).^2 - 20*Xif.*(tanh(Xif) + tanh(Yif) + tanh(Zif) )) .* exp(-10*(Xif.^2 + Yif.^2 + Zif.^2));
    
    Bje = (1 - tan(Yjf).^2 - 20*Yjf.*(tanh(Xjf) + tanh(Yjf) + tanh(Zjf) )) .* exp(-10*(Xjf.^2 + Yjf.^2 + Zjf.^2));
    
    Bke = (1 - tan(Zkf).^2 - 20*Zkf.*(tanh(Xkf) + tanh(Ykf) + tanh(Zkf) )) .* exp(-10*(Xkf.^2 + Ykf.^2 + Zkf.^2));

    Be = [Bie(:); Bje(:); Bke(:)];

    % |residual|
    curl3n = norm(GRADphi - Be, 'inf'); 
