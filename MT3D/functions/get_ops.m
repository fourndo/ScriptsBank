function [L_px,L_py,L_s,DIV_p,DIV_s,GRAD,CURL_e,CURL_f,AVFsc,AVFvcz,AVC,AVHx,AVHy,dZm]=get_ops(dx,dy,dz,w,s_background)
...
%% Laplacian operator for primary field y-polarized
...
%Will eventually need to compute kappa for individual cells since
%conductivity will change at every location
d_dz= dZm.^0.5 * ddx(nz); d_dz=d_dz(:,2:end-1); dd_dz= dZ * d_dz'*d_dz; 
...
L_py = [DDxx+DDyx+DDzx Oyx Ozx;
     Oxy DDxy+DDyy+DDzy Ozy;%
     Oxz Oyz DDxz+DDyz+DDzz];

%% Laplacian operator for primary field x-polarized
% Second partial Derivative in 3 directions
% %Partial derivatives for x-component
d_dx=  dXxl.^0.5 * ddx(nx+1); d_dx=d_dx(:,2:end-1); dd_dx= dXm * d_dx'*d_dx; dd_dx([1,end],:)=0;
DDxx = kron( kron( speye(nz) , speye(ny) ), dd_dx );
...
%% Averaging operator z-dir cell-center to faces (for cell-center 3d vector)

O = sparse( nfz , nx*nz*ny );
...
%% Averaging operator for H (edge to center)
